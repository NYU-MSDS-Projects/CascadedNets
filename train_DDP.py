...
def setup_dataset(args, rank, world_size):
  # Data Handler
  data_dict = {
  ...
  data_handler = DataHandler(**data_dict,rank=rank, world_size=world_size)
  print("Data handler loaded.")

    # Set Loaders
  loaders = {
      "train": data_handler.build_loader("train", args, rank=rank, world_size=world_size),
      "val": data_handler.build_loader("val", args,rank=rank, world_size=world_size),
      "test": data_handler.build_loader("test", args,rank=rank, world_size=world_size),
  }
  print("Dataloaders contructed.")
  return data_handler, loaders

  
def setup_loaders(args, data_handler,rank, world_size):
  # Set Loaders
  loaders = {
      "train": data_handler.build_loader("train", args, rank=rank, world_size=world_size),
      "val": data_handler.build_loader("val", args,rank=rank, world_size=world_size),
...
def train(rank,args,save_root,world_size):
  #innit dataprallel
  init_process(rank, world_size)
  print(f"Rank {rank}/{world_size-1} training process initialized.\n")
  print("current device {}".format(torch.cuda.current_device()))
  print("current rank {}".format(rank))

  device = rank

  print("current device after rank{}".format(torch.cuda.current_device()))
  # Setup dataset loader

  # if rank == 0:
  #   data_handler = setup_dataset(args,rank=rank, world_size=world_size)
  # dist.barrier()
  data_handler = setup_dataset(args,rank=rank, world_size=world_size)

  # loaders = setup_loaders(args, data_handler =data_handler,rank=rank, world_size=world_size)
...
def main(args):
  ...
  # Setup model
  save_root = ""
  mp.spawn(train, nprocs = world_size, args = (args,save_root,world_size))
